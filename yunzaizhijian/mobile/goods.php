...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$goods['total_price']  = get_final_price($goods['goods_id'], 1, true);
		$goods['goods_desc'] = str_replace('src="/images/', 'src="'.$config['site_url'].'images/', $goods['goods_desc']); //修复产品详情的图片 by wang
		//用户信息
		$user_info = $db->getRow("SELECT user_name,nick_name,user_id FROM " . $ecs->table('users') . " WHERE user_id=".$user_id);
		$smarty->assign('nick_name',          !empty($user_info['nick_name']) ? $user_info['nick_name'] : $user_info['user_name']);
