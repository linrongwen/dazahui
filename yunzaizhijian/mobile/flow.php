...
$smarty->assign('request_page', 'cart.php');
/*------------------------------------------------------ */
//-- PROCESSOR
/*------------------------------------------------------ */
...
elseif ($_REQUEST['step'] == 'change_bonus')
{
    ...
    else
    {
        ...
        if ((!empty($bonus) && $bonus['user_id'] == $user_id) || $_GET['bonus'] == 0)
        {
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if (empty($cart_goods))
    {
        show_message($_LANG['no_goods_in_cart'], $_LANG['back_home'], 'index.php', 'warning');
    }
   ...
   /* 判断是不是实体商品 */
    ...
    $db->query($sql);
    /* 如果已经余额、积分或者积分支付，分销处理 */
    if($order['order_amount'] <= 0){
        if($wxconf['is_auto_distribute']){
			$distri = new Distribute();
			$distri->deal($order['order_id']);
		}	
	}
...
elseif($_REQUEST['step'] == 'ok')
{
...

    {
        ...
        $payment = payment_info($order['pay_id']);
        if(!$payment || empty($payment['pay_code'])){
			show_message($_LANG['payment_no_exist']);
		} 
        include_once('include/modules/payment/' . $payment['pay_code'] . '.php');
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
    if ($user_id > 0)
    {
        ...
        $count = $db->getOne("SELECT goods_id FROM " . $ecs->table('collect_goods') . " WHERE user_id = '".$user_id."' AND goods_id = '$goods_id'");
        if (empty($count))
        {
            $time = gmtime();
...
else
{
    ...
    if ($user_id > 0)
    {
        ...
        $collection_goods = get_collection_goods($user_id);
...
function add_gift_to_cart($act_id, $id, $price)
{
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . " (" .
            ...
            "SELECT '".$user_id."', '" . SESS_ID . "', goods_id, goods_sn, goods_name, market_price, ".
...
function add_favourable_to_cart($act_id, $act_name, $amount)
{
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . "(" .
                "user_id, session_id, goods_id, goods_sn, goods_name, market_price, goods_price, ".
                "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) ".
            "VALUES('".$user_id."', '" . SESS_ID . "', 0, '', '$act_name', 0, ".
