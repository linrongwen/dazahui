...
function insert_slider(){
	$sql = 'SELECT ad_id,ad_name, ad_link, ad_code FROM ' . $GLOBALS['ecs']->table("touch_ad") . ' WHERE position_id=1 and '.time().'>start_time and '.time().'<end_time ORDER BY ad_sorts DESC LIMIT 5';
	$sliders = $GLOBALS['db']->getAll($sql, true);
	$GLOBALS['smarty']->assign('sliders', $sliders);
	$val = $GLOBALS['smarty']->fetch('library/ad_position.lbi');
