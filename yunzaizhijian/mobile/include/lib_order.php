...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 初始化要插入购物车的基本件数据 */
    $parent = array(
        'user_id'       => $GLOBALS['wechat']->get_userid(),
        'session_id'    => SESS_ID,
        'goods_id'      => $goods_id,
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['goods_price_noformat'] = $row['goods_price'];
		$row['market_price_noformat'] = $row['market_price'];
        $row['subtotal']     = price_format($row['goods_price'] * $row['goods_number'], false);
        $row['goods_price']  = price_format($row['goods_price'], false);
        $row['market_price'] = price_format($row['market_price'], false);
		$is_collect  =  $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('collect_goods') . " WHERE goods_id=$row[goods_id] AND user_id=".$row['user_id']."");
        if($is_collect>0){
		   	$row['is_collect'] = 1;
		}else{
		    $row['is_collect'] = 0;	
...
function flow_order_info()
{
    ...
    {
        /* 如果还没有设置配送和支付 */
        if ($GLOBALS['wechat']->get_userid() > 0)
        {
