...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, bank_account, bank_name, mobile_phone, real_name, nick_name ".
    ...
    $info['bonus']       = $bonus;
    $info['qq']          = $infos['qq'];
    $info['bank_account']   = $infos['bank_account'];
    $info['bank_name']= $infos['bank_name'];
    $info['mobile_phone']   = $infos['mobile_phone'];
    $info['mobile_phone'] = $infos['mobile_phone'];
    $info['real_name'] = $infos['real_name'];
    $info['nick_name'] = $infos['nick_name'];
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = '<span style="color:fff">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
            else
            {
        ...
        {
            $row['handler'] = '<span style="color:fff">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span>';
        }
...
function save_consignee($consignee, $default=false)
{
    ...
    {
        /* 修改地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_address'), $consignee, 'UPDATE', 'address_id = ' . $consignee['address_id']." AND `user_id`= '".$GLOBALS['wechat']->validate_user()."'");
    }
    else
    {
        /* 添加地址 */
...
function drop_consignee($id)
{
    ...
    if ($uid != $GLOBALS['wechat']->validate_user())
    {
        return false;
    }
    else
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {
        /* 判断是否在售后服务期 */
		$service_from_days = intval($GLOBALS['_CFG']['service_from_days']);
	    $service_end_days  = intval($GLOBALS['_CFG']['service_end_days']);
		$service_from_time = $service_from_days * 3600 * 24 + $order['shipping_time'];
		$service_end_time  += $service_end_days * 3600 * 24 + $order['shipping_time'];
		$cur_time = time();
		if($cur_time>$service_from_time && $cur_time<$service_end_time){
		   $order['is_service'] = 1;	
		}else{
		   $order['is_service'] = 0;	
		}
...
function return_to_cart($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (empty($cart_goods))
        {
            ...
            $return_goods['user_id']    = $GLOBALS['wechat']->validate_user();
