...
class cls_image
{
	 ...
	 * @param   string      $imgtype         指定生成图片的类型
     * @return  mix         如果成功返回缩略图的路径，失败则返回false
     */
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $imgtype='')
    {
        ...
        /* 生成文件 */
        if($imgtype!=''){
		    $creat_fun = "image" . $imgtype;
			$filename .= '.' . $imgtype;
			$creat_fun($img_thumb, $dir . $filename);	
		}else{
     ...
     * @param       string      target_file         需要加水印的图片文件名，包含完整路径。如果为空则覆盖源文件
     * @param       string      $watermark          水印完整路径
     * @param       int         $watermark_posX     水印X坐标位置
	 * @param      int          $watermark_posY      水印y坐标位置
     * @return      mix         如果成功则返回文件路径，否则返回false
     */
    public function add_watermark_position($filename, $target_file='', $watermark='', $watermark_posX=0, $watermark_posY=0,  $watermark_alpha = 0.65)
    {
        // 是否安装了GD
        $gd = $this->gd_version();
        if ($gd == 0)
        ...
        /* 如果水印的位置为0，则返回原图 */
        if ($watermark_posX == 0 || empty($watermark))
        {
            return str_replace(ROOT_PATH, '', str_replace('\\', '/', realpath($filename)));
        }
	    ...
	    if(!$width || !$height){	
          $watermark_info     = @getimagesize($watermark);
		  $width = $watermark_info[0];
		  $height = $watermark_info[1];
		}
        $watermark_handle   = $this->img_resource($watermark, $watermark_info[2]);
        ...
        {
            imageAlphaBlending($watermark_handle, true);
            imagecopy($source_handle, $watermark_handle, $x, $y, 0, 0,$width, $height);
        }
        else
        {
            imagecopymerge($source_handle, $watermark_handle, $x, $y, 0, 0,$width, $height, $watermark_alpha);
        }
        $target = empty($target_file) ? $filename : $target_file;
	...
	*/
	function add_txt($filename, $font=16, $ang=0, $x=1, $y=1, $color=array(0,0,0), $fontface='C://WINDOWS//Fonts//simhei.ttf', $string='' )
	{
	    list($width, $height, $type) = getimagesize($filename);
	   	$types = array(1=>"gif", 2=>"jpeg", 3=>"png");
		$createfrom = "imagecreatefrom" . $types[$type];
		//新建一个可操作的图片对象
		$img = $createfrom($filename);
		$textcolor = imagecolorallocate($img, $color[0], $color[1], $color[2]);
		imagettftext($img, $font, $ang, $x, $y, $textcolor, $fontface, $string);
	...
	//获取远程图片并保存至本地
	function copy_web_image($webimg, $path){
			$qrimg = $this->curl_get_contents($webimg);
