...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        {
            log_account_change($GLOBALS['wechat']->validate_user(), 0, 0, $GLOBALS['_CFG']['register_points'], $GLOBALS['_CFG']['register_points'], $GLOBALS['_LANG']['register_points']);
        }
            ...
            if ($up_uid)
            {
                if (!empty($affiliate['config']['level_register_all']))
                {
                    if (!empty($affiliate['config']['level_register_up']))
                    {
                        ...
                        {
                            log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], 0, sprintf($GLOBALS['_LANG']['register_affiliate'], $GLOBALS['wechat']->validate_user(), $username));
                        }
                    }
                    else
                    {
                ...
                //设置推荐人
                $sql = 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $GLOBALS['wechat']->validate_user();
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $update_data, 'UPDATE', 'user_id = ' . $GLOBALS['wechat']->validate_user());
