    ...
    if ($_GET['act'] == 'asynclist') {
        ...
        if (is_array($goodslist)) {
            foreach ($goodslist as $vo) {
                ...
                $sayList[] = array(
                    'pro-inner' => '
        <div class="proImg-wrap"> <a href="'.$vo['url'].'&wxid='.$wxid.'" > <img src="'.$config['site_url'].'mobile/'.$vo['goods_img'].'" alt="'.$vo['goods_name'].'"> </a> </div>
        <div class="proInfo-wrap">
          <div class="proTitle"> <a href="'.$vo['url'].'&wxid='.$wxid.'" >'.$vo['name'].'</a> </div>
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot,g.original_img, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['goods_img']        = get_image_path($row['goods_id'], $row['original_img']);
...
function get_sales_volume($goods_id)
{
	...
	//加上初始销量
	$init_sale_count = $GLOBALS['db']->getOne("SELECT init_sale_count FROM ".
            $GLOBALS['ecs']->table('goods') ." WHERE goods_id=".$goods_id);
	$sales_count += $init_sale_count;
    return $sales_count;
}
