...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, '.
                ...
                'g.promote_price, g.promote_start_date,g.promote_end_date, c.rec_id, c.is_attention, c.add_time' .
    ...
    $res = $GLOBALS['db'] -> selectLimit($sql, $num=10, $start=0);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['add_time'] = date('Y-m-d h:i:s', $row['add_time']);
...
{
	if(empty($surplus['admin_note'])){
	    	$surplus['admin_note'] = '';
	}
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type,real_name,bank_name,bank_account,mobile,payment, is_paid)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '$surplus[admin_note]', '$surplus[user_note]', '$surplus[process_type]','$surplus[real_name]','$surplus[bank_name]','$surplus[bank_account]','$surplus[mobile]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
...
function get_user_default($user_id)
{
	...
	$info['subscribe_time'] = date('Y-m-d', $row['subscribe_time']);
