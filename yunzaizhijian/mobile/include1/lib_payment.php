    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            if ($pay_log['order_type'] == PAY_ORDER)
            {
                ...
                $GLOBALS['db']->query($sql);
				//自动分销操作
                if($GLOBALS['wxconf']['is_auto_distribute']){
					$distri = new Distribute();
				   	$distri->deal($order_id);
				}
                /* 记录订单操作记录 */
            ...
            {
				//更新伪订单
				$sql = "UPDATE " . $GLOBALS['ecs']->table('order_info')." SET pay_status=2,order_status=1, confirm_time=".time().", pay_time=".time()." WHERE order_id=".$pay_log['order_id'];
				$GLOBALS['db']->query($sql);
				//余额改为已通过
                $sql = 'SELECT `id` FROM ' . $GLOBALS['ecs']->table('user_account') .  " WHERE `id` = ".$pay_log[user_account_id]." AND `is_paid` = 1  LIMIT 1";
                ...
                if(empty($res_id))
                {
                    ...
                    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('user_account') .
                           ...
                           " WHERE id = $pay_log[user_account_id] LIMIT 1";
                    $GLOBALS['db']->query($sql);

                    /* 取得添加预付款的用户以及金额 */
                    $sql = "SELECT user_id, amount,admin_note FROM " . $GLOBALS['ecs']->table('user_account') .
                            " WHERE id = '$pay_log[user_account_id]'";
                    $arr = $GLOBALS['db']->getRow($sql);
                    /* 修改会员帐户金额 */
                    $_LANG = array();
                    include_once(ROOT_PATH . 'lang/' . $GLOBALS['_CFG']['lang'] . '/user.php');
                    log_account_change($arr['user_id'], $arr['amount'], 0, 0, 0, $arr['admin_note'], ACT_SAVING);
                }
            }
        }
        else
