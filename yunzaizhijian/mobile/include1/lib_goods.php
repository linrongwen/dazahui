...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show, cat_ico ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
			   ...
			   $three_arr[$row['cat_id']]['ico'] = empty($row['cat_ico']) ? $GLOBALS['_CFG']['no_picture'] : DATA_DIR.'/cat_ico/'.$row['cat_ico'];
...
{
    if (!in_array($type, array('best', 'new', 'hot', 'join')))
    {
        return array();
    }
    ...
    if (empty($type_goods[$type]))
    {
		...
		$type_goods['join'] = array();
        $data = read_static_cache('recommend_goods');
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_join, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 or g.is_join = 1)'.
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
					...
					if ($data['is_join'] == 1)
                    {
                        $goods_data['join'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'join'=>'recommend_join');
        if (empty($type_array))
        {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.original_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'], $type_array['join']);
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['thumb']        = get_image_path($row['goods_id'], 'mobile/'.$row['original_img'], true);
            $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], 'mobile/'.$row['goods_img']);
			...
			if (in_array($row['goods_id'], $type_array['join']))
            {
                $type_goods['join'][] = $goods[$idx];
            }
        }
    }
