...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
		...
		$is_auto_distribute = $db->getOne("SELECT is_auto_distribute FROM wxch_config");
		if($is_auto_distribute == 1)
		  echo("<script>location.href='order.php?id=".$order_id."&act=distribute_order';</script>");
    }
    /* 设为未付款 */
	...
	elseif ('change' == $operation)
    {
	/* 记录log */
        order_action($order['order_sn'], $order['order_status'], SS_CHANGED , $order['pay_status'], '【换货】 ' . $action_note);
    }
...
//分销审核
elseif ($_REQUEST['act'] == 'distribute_order'){
    /* 检查权限 */
    admin_priv('order_edit');
    ...
    $reg = $distri->deal($order_id);
	if($reg['status'] == 0){
		sys_msg($reg['msg']);
	}
	/* 记录log */
    order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], '[' . $_LANG['op_distribute'] . '] ' . $action_note);
	$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
	sys_msg($GLOBALS['_LANG']['act_ok'], 1, $links);
}
/* 取消分销 */
elseif ($_REQUEST['act'] == 'cancel_distribute'){
	/* 检查权限 */
    admin_priv('order_edit');
	...
	require_once(ROOT_PATH . 'include/cls_distribute.php');
	$distri = new Distribute();
    $reg = $distri->cancel_distribute($order_id);
	if($reg['status'] == 0){
		sys_msg($reg['msg']);
	}
	/* 记录log */
    order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], '[取消分销] ' . $action_note);
	$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
	sys_msg($GLOBALS['_LANG']['act_ok'], 1, $links);
}
/*------------------------------------------------------ */
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['distribute_status'] = isset($_REQUEST['distribute_status']) ? intval($_REQUEST['distribute_status']) : 0;
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.distribute_status, o.order_amount, o.money_paid," .
