...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, is_show, sort_order , brand_banner, one_level_deduct, two_level_deduct, three_level_deduct) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$is_show', '$_POST[sort_order]' , '$_POST[banner_name]', '$_POST[one_level_deduct]', '$_POST[two_level_deduct]', '$_POST[three_level_deduct]')";
    $db->query($sql);
     /**by ecmoban E*/
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT * FROM " .$ecs->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]', one_level_deduct='$_POST[one_level_deduct]', two_level_deduct='$_POST[two_level_deduct]', three_level_deduct='$_POST[three_level_deduct]' ";
    //if (!empty($img_name))
    //{
        //有图片上传
