...
elseif($_REQUEST['act'] == 'add') 
{
	...
	if($_POST)
	{
		...
		if($ticket)
		{
			...
			$qr_conf = $db->getRow("SELECT * FROM wxch_qr_set");
			require_once(ROOT_PATH . 'include/cls_image.php');
	        $image = new cls_image();
			$qr_thumb = $image->make_thumb(ROOT_PATH . $qr_path , $qr_conf['qr_width'],  $qr_conf['qr_height'], ROOT_PATH . 'images/qrcode_thumb/','', 'png');
			if(@filesize(ROOT_PATH.$qr_path)<10240) 
			{
				$qr_path = '';
			}
			$insert_sql = "INSERT INTO `wxch_qr` (`type`,`expire_seconds`,`action_name`,`ticket`, `scene_id`, `scene` ,`qr_path`, `qrcode_thumb`,`function`) VALUES
   ('$type','$expire_seconds','$action_name', '$ticket',$scene_id, '$scene' ,'$qr_path','$qr_thumb','$function')";
			$db->query($insert_sql);
...
elseif($_REQUEST['act'] == 'edit') 
{
	if($_POST)
	{
		...
		if($ticket)
		{
			...
			$update_sql = "UPDATE  `wxch_qr` SET  `scene` =  '$scene',`function` = '$function',`expire_seconds` = '$expire_seconds',`qr_path` = '$qr_path', `qrcode_thumb`='$qr_thumb' WHERE  `qid` ='$id';";
			$db->query($update_sql);
			$link[] = array('href' =>'wxch-ent.php?act=qr', 'text' => $wxch_lang['ur_here']);
			sys_msg('修改成功',0,$link);
		}
