...
//-- 修改分销商状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_distributor')
{
    /* 检查权限 */
    check_authz_json('users_manage');
    ...
    $sql = "SELECT is_distributor,user_name FROM " . $ecs->table('users') . " WHERE user_id = '$id'";
    $uinfo = $db->getRow($sql);
    if($uinfo[is_distributor]){
	   	$db->query("UPDATE " . $ecs->table('users') . " SET is_distributor=0 WHERE user_id = '$id'");
		admin_log(addslashes($uinfo['user_name']), 'edit', 'users');
		make_json_result(stripcslashes(0));
	}
	else{
	   	$db->query("UPDATE " . $ecs->table('users') . " SET is_distributor=1 WHERE user_id = '$id'");
		admin_log(addslashes($uinfo['user_name']), 'edit', 'users');
		make_json_result(stripcslashes(1));
	}
}
...
elseif ($_REQUEST['act'] == 'user_detail'){
	...
	if($user['affiliate_id']>0){
		   $first_affiliate = $GLOBALS['db']->getRow("SELECT head_url,nick_name FROM " . $GLOBALS['ecs']->table('users')." WHERE user_id=".$user['affiliate_id']);
		   $user['first_head_url']	=  $first_affiliate['head_url'];
		   $user['first_nick_name'] =  $first_affiliate['nick_name'];
	}
	//二层上级
	if($user['second_affiliate_id']>0){
		   $second_affiliate = $GLOBALS['db']->getRow("SELECT head_url,nick_name FROM " . $GLOBALS['ecs']->table('users')." WHERE user_id=".$user['second_affiliate_id']);
		   $user['second_head_url']	=  $second_affiliate['head_url'];
		   $user['second_nick_name'] =  $second_affiliate['nick_name'];
	}
	//三层上级
	if($user['third_affiliate_id']>0){
		   $third_affiliate = $GLOBALS['db']->getRow("SELECT head_url,nick_name FROM " . $GLOBALS['ecs']->table('users')." WHERE user_id=".$user['third_affiliate_id']);
		   $user['third_head_url']	=  $third_affiliate['head_url'];
		   $user['third_nick_name'] =  $third_affiliate['nick_name'];
	}
	$user['first_affiliate_persons']  = $distri->affiliate_counts($user['user_id'], 'affiliate_id');
	$user['second_affiliate_persons'] = $distri->affiliate_counts($user['user_id'], 'second_affiliate_id');
	$user['third_affiliate_persons']  = $distri->affiliate_counts($user ['user_id'], 'third_affiliate_id');
	$user['all_affiliate_persons'] = $user['first_affiliate_persons'] + $user['second_affiliate_persons'] + $user['third_affiliate_persons'];
	assign_query_info();
    $smarty->assign('ur_here',          $_LANG['user_detail']);
    $smarty->assign('action_link',      array('text' => $_LANG['03_users_list'], 'href'=>'users.php?act=list&' . list_link_postfix()));
    $smarty->assign('user',             $user);
...
//--  用户收藏查看
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'user_favourite')
{
	require_once(ROOT_PATH . 'include/lib_clips.php');
    $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    $fav_list = get_collection_goods($id, 50);
    assign_query_info();
	$smarty->assign('fav_list',         $fav_list);
    $smarty->assign('ur_here',          $_LANG['favourite_list']);
    $smarty->assign('action_link',      array('text' => $_LANG['03_users_list'], 'href'=>'users.php?act=list&' . list_link_postfix()));
    $smarty->display('user_favourite.htm');
}
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, nick_name, head_url, is_subscribe, is_distributor, subscribe_time, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time  ".
