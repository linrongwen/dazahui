...
if ($_REQUEST['act'] == 'list')
{
	...
	foreach($user_list1 as $aa){
		$user_id=$aa['user_id'];
		$sql = "SELECT count('uid')  FROM wxch_user wu where wu.affiliate='$user_id' ";
		$number = $db->query($sql);
    ...
    assign_query_info();
    $smarty->display('users_list_invite.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('users_list_invite.htm'), '', array('filter' => $user_list['filter'], 'page_count' => $user_list['page_count']));
}
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ecs_users as eu right join wxch_qr as wq on wq.affiliate = eu.user_id  " . $ex_where." and eu.user_id is not null");

        /* 分页大小 */
		$filter = page_and_size($filter);
/*         $sql = "SELECT eu.user_id,wu.affiliate, eu.user_name, eu.email, eu.is_validated, eu.user_money, eu.frozen_money, eu.rank_points, eu.pay_points, eu.reg_time FROM ecs_users  eu,wxch_user wu where (wu.wxid = eu.wxid )ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];  */
				
			$sql = "SELECT * FROM ecs_users as eu right join wxch_qr as wq on wq.affiliate = eu.user_id ".$ex_where." and eu.user_id is not null  ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
	
			/* $sql = "SELECT eu.user_id, eu.user_name, eu.email, eu.is_validated, eu.user_money, eu.frozen_money, eu.rank_points, eu.pay_points, eu.reg_time FROM ecs_users  eu  ,wxch_qr wq where ( wq.affiliate = eu.user_id) ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];  */
